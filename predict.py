...
class Predictor:
    ...
    def setup(self) -> None:
        ...
        print_log(
            f"• Dtype: {torch.bfloat16 if self.device.type == 'cuda' else torch.float32}"
        )
        print_log(
            f"• Device Map: {'auto' if self.device.type == 'cuda' else 'None'}"
        )
        print_log(
            f"• Quantization: {'8-bit' if self.device.type == 'cuda' else 'None'}"
        )
    ...
    async def main_setup(self, model_id):
        ...
        await asyncio.gather(
            self.setup_model(model_id),  # Load model asynchronously
            self.setup_processor(
                model_id)  # Load processor asynchronously (if needed)
        )

    async def setup_model(self, model_id):
        # Load model with minimal settings and BF16 for faster loading
        try:
            self.model = await asyncio.to_thread(
                VideoLlavaForConditionalGeneration.from_pretrained,
                model_id,
                torch_dtype=torch.bfloat16
                if self.device.type == "cuda" else torch.float32,
                device_map="auto" if self.device.type == "cuda" else None,
                load_in_8bit=True if self.device.type == "cuda" else False,
    ...
    async def setup_processor(self, model_id):
        # Load processor
        try:
            self.processor = await asyncio.to_thread(
                VideoLlavaProcessor.from_pretrained, model_id)
            print_log("Processor loaded successfully.")
        except Exception as e:
            print_log(f"Error during processor setup: {str(e)}")
            raise RuntimeError(f"Processor setup failed: {str(e)}") from e
    ...
    def predict(
        ...
        try:
            ...
            container = av.open(
                str(video)
            )  # video is already a string now, but str() ensures safety
            ...
            if total_frames == 0:
                total_frames = sum(
                    1 for _ in container.decode(video=0))  # Count manually
                container.seek(0)  # Reset after counting

            # Adjust frames_to_use based on available frames in the video
            frames_to_use = min(total_frames,
                                num_frames) if total_frames > 0 else num_frames
            ...
            print_log(
                f"Total prediction time: {time.time() - predict_start:.2f}s")
